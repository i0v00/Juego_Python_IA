...
def leer_datos():
    ...
    try:
        with open("datos.txt", "r") as f:
            for linea in f:
                if "MatrizX=" in linea:
                    MatrizX = int(linea.split("=")[1].strip())
                elif "MatrizY=" in linea:
                    MatrizY = int(linea.split("=")[1].strip())
    except FileNotFoundError:
        print("El archivo de datos no se encontró. Asegúrate de que 'ingresar_matrices.py' haya sido ejecutado.")
    except ValueError:
        print("Error al leer los datos. Asegúrate de ingresar números válidos.")
    return MatrizX, MatrizY

# Llamar a la función para leer los datos
MatrizX, MatrizY = leer_datos()
print("MatrizX recibida:", MatrizX)
print("MatrizY recibida:", MatrizY)
...
if MatrizX==2 or MatrizX==1:
    TMatrizX=3
if MatrizY>=5 and (MatrizX>=2 and MatrizX<=4):
    TMatrizY=10


Promedio_Matriz=(TMatrizX+TMatrizY)/2
int(Promedio_Matriz)
Tamaño_Imagen_Matriz=800/Promedio_Matriz
int(Tamaño_Imagen_Matriz)

altura_boton = 30  # El botón de abajo, para iniciar juego
medida_cuadro = 800/Promedio_Matriz  # Medida de la imagen en pixeles
...
# Escala la imagen segun la pantalla lo vea necesario, calculando en un promedio de 800px
imagen_oculta = pygame.transform.scale(imagen_oculta, (Tamaño_Imagen_Matriz, Tamaño_Imagen_Matriz))
...
class Cuadro:
    def __init__(self, fuente_imagen):
        ...
        # Escala la imagen a 200x200 píxeles
        self.imagen_real = pygame.transform.scale(self.imagen_real, (Tamaño_Imagen_Matriz, Tamaño_Imagen_Matriz))


"""
Todo el juego; que al final es un arreglo de objetos
"""
# Lista de imágenes disponibles (hasta 8 pares)
...
# Calcula el número total de pares necesarios
numero_de_pares = (MatrizX * MatrizY) // 2

# Crear una lista de imágenes para pares (repetir y cortar según el número de pares necesarios)
imagenes_pares = (imagenes_disponibles * ((numero_de_pares // len(imagenes_disponibles)) + 1))[:numero_de_pares]

# Duplicar cada imagen para crear los pares
imagenes_para_matriz = imagenes_pares * 2

# Mezclar las imágenes
random.shuffle(imagenes_para_matriz)

# Crear la estructura de la matriz basada en MatrizX y MatrizY
cuadros = []
for y in range(MatrizY):
    fila = []
    for x in range(MatrizX):
        imagen = imagenes_para_matriz.pop()  # Extrae la última imagen de la lista
...
# Los sonidos
sonido_fondo = pygame.mixer.Sound("assets/fondo.mp3")
sonido_fondo.set_volume(0.08)
sonido_clic = pygame.mixer.Sound("assets/clic.mp3")
sonido_clic.set_volume(0.8)
sonido_exito = pygame.mixer.Sound("assets/ganador.mp3")
sonido_fracaso = pygame.mixer.Sound("assets/equivocado.mp3")
sonido_fracaso.set_volume(0.5)
sonido_voltear = pygame.mixer.Sound("assets/voltear.wav")
sonido_voltear.set_volume(1)
...
def comprobar_si_gana():
    if gana():
        ...
        sonido_clic.set_volume(0)
        sonido_exito.set_volume(0.6)
        sonido_fracaso.set_volume(0)
        sonido_voltear.set_volume(0)
        reiniciar_juego()
...
pygame.display.set_caption('Memorama Kolla - Grupo 1')
